...
template <typename T> class INSGA : public Evolutionary_Algorithm<T>
{
    ...
    Population extend_population(const Population& base_population, const Population& offspring_population) const;
    Fronts non_dominance_sorting(const Population& base_extended_population) const;
    ...
    Population generate_init_pop(population_generator<T> generator,
                                             int pop) override;
    Population crossover(strategy<T> strat,
                                     Population population,
                                     const std::vector<float>& params) override;
    Front solve(int popsize, int iterations, target_function<T> target, strategy<T> cross_strat,
