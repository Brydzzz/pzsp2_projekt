...
class SPEA2Wrapper : SPEA2<T> {
  public:
    ...
    class SPEA2Wrapper : SPEA2<T> {
      public:
        SPEA2Wrapper(std::function<float(T, T)> distance)
            : SPEA2<T>(distance) {}
...
TEST(SPEA2Tests, test_calculate_strength) {
    ...
    TEST(SPEA2Tests, test_calculate_distances) {
        ...
        std::vector<int> pop = {1, -2, 3, 0};
        std::pair<std::vector<float>, std::vector<std::vector<float>>>
            distances = spea2.wrap_calculate_distances(pop);
    ...
    TEST(SPEA2Tests, test_get_newset_not_enough) {
        ...
        std::vector<int> set = {3, 0};
        auto fitness_result =
            spea2.wrap_calculate_fitness(mock_target, set, pop);
        std::vector<int> newset =
            spea2.wrap_get_newset(3, mock_target, pop, set,
                                  fitness_result.second, fitness_result.first);
    ...
    TEST(SPEA2Tests, test_get_newset_too_many) {
        ...
        auto fitness_result =
            spea2.wrap_calculate_fitness(mock_target, set, pop);
        std::vector<int> newset =
            spea2.wrap_get_newset(1, mock_target, pop, set,
                                  fitness_result.second, fitness_result.first);
